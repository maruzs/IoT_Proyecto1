...
class MQTTPublisher:
    ...
    Follows the same v1/v2 compatibility pattern and topic_base convention
    as the backend MQTTClient.
    """
    ...
    def __init__(
        ...
        # paho-mqtt v1 / v2 compatibility — same pattern as backend
        try:
            self.client = mqtt.Client(
                client_id=f"llm-gateway-{self.equipo_id}",
            )
        except TypeError:
            self.client = mqtt.Client(
    ...
    def publish(self, topic_suffix: str, payload: dict) -> bool:
        """Publish a JSON payload to ``{topic_base}/{topic_suffix}`` with QoS 1.
