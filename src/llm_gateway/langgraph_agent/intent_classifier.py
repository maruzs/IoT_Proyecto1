...
async def _llm_deduce(message: str, ollama_client) -> dict:
    ...
    system_prompt = (
        ...
        "- \"¿hay alguien en la puerta?\", \"¿quién está afuera?\" → trigger_camera\n"
        "- \"¿se escucha algo?\", \"¿golpearon la puerta?\" → get_sensor_state (ruido)\n"
        "- \"¿hace calor?\", \"¿hay gas?\" → get_sensor_state (temp/gas)\n"
        "- \"abrime\", \"cerrá la puerta\" → activate_led_puerta\n"
        "- \"avisame\", \"mandá mensaje\" → send_notification\n"
        "- \"callate\", \"basta\" → silence_alerts\n"
        "- \"¿qué pasó antes?\", \"historial\" → query_history\n\n"
